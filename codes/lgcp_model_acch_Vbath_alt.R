...
nsamp <- 500 # Define number of sampling for the final predictions
nsampcv <- 500 # Define number of sampling in the CV predictions
...
plot.res("pa", pred.fit)+gg(pa.pts, aes(col = presence))+gg(po.pts, col = "white")
...
#                           )), n.samples = nsampcv)
#     
#     cv.boyce[k] <- ecospat::ecospat.boyce(cv.pred.pa$mean, cv.pred.pa$mean[pa.pts[pa.blocks == k,]$presence ==1])$cor
#     
#     brier[k] <- mean((cv.pred.pa$mean-pa.pts[pa.blocks == k,]$presence)^2)
#     
#     test.po <- po.pts[po.blocks == k,]
#     test.ips <- ips.blocks[ips.blocks$ID == k,]
#     
#     fdat <- rbind(test.po, test.ips)
#     
#     cv.pred.pa.f <- predict(blockm, fdat,
#                           as.formula(paste0(
#                             "~1-",
#                             update(forms[[z]], ~ exp(-exp(. + intercept_pa + spatial_pa)))[2]
#                           )), n.samples = nsampcv)
...
### brier test
# fdat <- rbind(po.pts, ips)
# 
# pred.prob <- predict(m[[1]], fdat,
#                      as.formula(paste0(
#                        "~1-",
#                        update(forms[[1]], ~ exp(-exp(. + intercept_pa + spatial_pa)))[2]
#                      )), n.samples = 1000)
# 
# pred.prob <- predict(m[[1]], pa.pts,
#                      as.formula(paste0(
#                        "~1-",
#                        update(forms[[1]], ~ exp(-exp(. + intercept_pa + spatial_pa)))[2]
#                      )), n.samples = 1000)
# 
# mean((pred.prob$mean-pa.pts$presence)^2)
# 
# ecospat::ecospat.boyce(pred.prob$mean, pred.prob$mean[1:length(po.pts)])
# 
# ecospat::ecospat.boyce(pred.prob$mean, pred.prob$mean[pa.pts$presence == 1])
