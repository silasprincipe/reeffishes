...
metrics <- c("q0.5", "q0.025", "q0.975", "mean", "sd")
...
for (j in 1:length(types)) {
  ...
  for (m in metrics) {
    ...
    if (m == "sd") {
      # Set color scale
      step.guide <- guide_colorbar(title = "SD(Probability of occurrence)",
                                   show.limits = TRUE,
    ...
    for (g in 1:length(groups)) {
      ...
      for (i in 1:length(species)) {
        ...
        preds <- as.data.frame(preds, xy = T)
        
        preds <- pivot_longer(preds,
                              cols = 3:7, names_to = "scenario", values_to = "value")
        ...
        if (i == 1) {
          ...
          pred.data <- rbind(pred.data, preds)
        }
      }
      ...
      ggsave(plot = p,
             filename = paste0("figures/", group.name, "_", gsub("\\.", "-", m), "_", type, ".jpg"),
