...
g.model <- inla.spde2.pcmatern(mesh,
                               prior.range = c(range0, 0.01),
                               prior.sigma = c(sigma, 0.01))
...
cmp <- list(
  ~ tempmax(env.e, model = d1spde.st, main_layer = "tempmax") +
    ...
    lgcp_spatial(coordinates, model = b.model, mapper = bru_mapper(mesh)) +
    spatial(coordinates, model = g.model, mapper = bru_mapper(mesh)) +
...
for (i in 1) {
  ...
  lik.lgcp <- like(data = po.pts,
                   ...
                   formula = update(forms[[i]], coordinates ~ . + Intercept + spatial + lgcp_spatial))
...
pred.fit <- predict(m[[tm]], pxl,
                    ~ list(
                      full = eval(parse(text = paste0("exp(",
                                                      update.formula(pred.f, ~ . + Intercept + spatial + lgcp_spatial)[2],
                                                      ")"))),
